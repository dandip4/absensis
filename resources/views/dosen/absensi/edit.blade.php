            ...
            <div class="row justify-content-center">
                <div class="col-md-8">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form name="form-edit" id="form-edit" action="{{ route('absensi.update', $absensi->id) }}" method="POST" class="needs-validation" enctype="multipart/form-data" novalidate>
                                @csrf
                                @method('PUT')
                                @php
                                    $user = auth()->user();
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select class="form-control select2" id="mata_kuliah_id" name="mata_kuliah_id" required>
                                                    @foreach($mataKuliahs as $mataKuliah)
                                                        <option value="{{ $mataKuliah->id }}" {{ $absensi->mata_kuliah_id == $mataKuliah->id ? 'selected' : '' }}>{{ $mataKuliah->nama }}</option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2" id="week_id" name="week_id" required>
                                                @foreach($weeks as $week)
                                                    <option value="{{ $week->id }}" {{ $absensi->week_id == $week->id ? 'selected' : '' }}>{{ $week->nama }}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="datetime-local" class="form-control" id="start_time" name="start_time" value="{{ \Carbon\Carbon::parse($absensi->start_time)->format('Y-m-d\TH:i') }}" required>
                                            <div class="invalid-feedback">Waktu Mulai Absen wajib diisi.</div>
                                        </div>

                                        <div class="form-group">
                                            <label for="deadline_time">Batas Waktu Absen</label>
                                            <input type="datetime-local" class="form-control" id="deadline_time" name="deadline_time" value="{{ \Carbon\Carbon::parse($absensi->deadline_time)->format('Y-m-d\TH:i') }}" required>
